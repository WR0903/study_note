...
int StereoCalib::detectCorners(cv::Mat& img1, cv::Mat& img2, CornerDatas& cornerDatas, int imageCount)
{
	...
	// 显示当前good_board的数目
	char info[10];
	sprintf(info, "%02d/%02d", imageCount + 1, cornerDatas.nImages);
	string text = info;
	showText(img1, text);
	if (stereoMode)
		showText(img2, text);
...
int StereoCalib::saveCameraParams(const CameraParams& cameraParams, const char* filename /* = "cameraParams.yml" */)
{
	...
	if (fs.isOpened())
	{
		...
		int nImages = (int)cameraParams.rotations.size();
		fs << "nImages" << nImages;
		for (int i = 0; i < nImages; i++)
		{
			char matName[50];
...
int StereoCalib::calibrateStereoCamera(CornerDatas& cornerDatas, StereoParams& stereoParams, bool cameraUncalibrated /* = false */)
{
	if (cameraUncalibrated)
	{
		...
		*	保存单目定标结果至本地
		*/
        saveCameraParams(stereoParams.cameraParams1, "/home/wr/WHR/StereoCamera-master/new/HD/cameraParams_left.yml");
        saveCameraParams(stereoParams.cameraParams2, "/home/wr/WHR/StereoCamera-master/new/HD/cameraParams_right.yml");
	}
